...
(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      #${rootId} .tapis-kpi-icon{display:grid;place-items:center;width:2.6rem;height:2.6rem;border-radius:.5rem;background:color-mix(in srgb,var(--tapis-kpi-color,var(--tapis-primary)) 14%,white);color:var(--tapis-kpi-color,var(--tapis-primary));line-height:0;flex:0 0 auto}
      ...
      #${rootId} .tapis-kpi-copy{display:block;min-width:0}
      #${rootId} .tapis-kpi-copy span{display:block;color:var(--tapis-muted);font-size:.72rem;font-weight:600;text-transform:uppercase}
      #${rootId} .tapis-kpi-copy strong{display:block;margin:.2rem 0 0;color:var(--tapis-text);font-size:1.45rem;font-weight:600;line-height:1.05;letter-spacing:0}
  ...
  function summaryCard(label, value, icon, tone) {
    return `
      <div class="tapis-kpi ${esc(tone)}">
        ...
        <span class="tapis-kpi-copy"><span>${esc(label)}</span><strong>${esc(value)}</strong></span>
      </div>
    `;
  }
